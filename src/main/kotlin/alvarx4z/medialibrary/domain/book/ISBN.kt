...
data class ISBN(
  val standardId: NotEmptyString,
  val legacyId: NotEmptyString?
) {
  init {
    checkStandardIsbn()
    checkLegacyIsbn()
  ...
  private fun checkStandardIsbn() = require(standardFormatRegex.matches(standardId.value)) { INVALID_MESSAGE }

  private fun checkLegacyIsbn() =
    legacyId?.value?.let {
      require(legacyFormatRegex.matches(it)) { INVALID_MESSAGE }
    }
